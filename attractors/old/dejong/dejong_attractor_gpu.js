...
let ContDeJongGPU = class
{
    constructor(np, kick, s)
    {
        this.Rate = 0.02;
        this.PMin = [-2.5, -2.5, -2.5, -.5];
        this.PMax = [2.5, 2.5, 2.5, 2.5];
        ...
        this.qexp_kernel = gpu.createKernel(
            function(points, params, s)
            {
                ...
                const x = points[this.thread.x][0];
                const y = points[this.thread.x][1];
                const new_x = Math.sin(A*y) - Math.cos(B*x)
                const new_y = Math.sin(C*x) - Math.cos(D*y)
                return [x + s*(new_x-x), y + s*(new_y-y)];
            },
            { output: [this.NP] }
        );
        ...
        {
            //return [this.normal(), this.normal()];
            //return [3*(Math.random()*2-1), 3*(Math.random()*2-1)];
            if( Math.random() < 0.25 )
                return [this.normal()+1, this.normal()-1];
    ...
    step(params)
    {
        ...
        this.Points = this.qexp_kernel(this.Points, params, this.S);
        return this.Points;
    }
}
...
let ContDeJong = class
{
    constructor(canvas_element)
    {
        this.NP = 50000;
        this.D = new ContDeJongGPU(this.NP, 0.02);
        this.margin = 0;
        const w = window.innerWidth;
        const h = window.innerHeight;
        const xmin = this.D.XMin;
