...
function bip39_generate(nwords)
{
    if( nwords % 3 )
        throw("Word count must be a multiple of 3");
    
    let n32 = nwords/3;
    let ENT = n32 * 32;
    let CS = ENT/32;
    
    const entropy = new Uint32Array(n32);
    window.crypto.getRandomValues(entropy);

    var zeros = [];
    for(let i = 0; i < n32; i++)
        zeros.push(0x7f7f7f7f);
    
    var entropy_bits = new BitStream(entropy);
    const sha = sha256(entropy_bits.Buffer, false);
    const sha_hex = sha256(entropy_bits.Buffer, true);
    var sha_bits = new BitStream(sha);
    const checksum = sha_bits.peek(CS);
    entropy_bits.push(checksum);
    
    var words = [];
    while( nwords-- > 0 )
        words.push(BIP39[entropy_bits.pull(11).as_int()]);
